...
#:
#: There are five classes, not four, and they share this curve only up to level 99.
#: At 100 the warrior's plateaus — 882229601, then one more point per level, which
#: reads as a sentinel keeping the thresholds strictly increasing — while the other
#: four carry on rising to 1197819860 at 110. Hit points and damage are five distinct
#: curves, one per class, at every level. An earlier note here claimed four classes
#: sharing everything but hit points, and it was wrong on both counts.
#:
#: 110 levels. The table used to stop at 30, so a character above that fell off the
#: end of it: level_for returned 30 and damage_at and hit_points_at clamped there.
LEVEL_EXPERIENCE = (
    ...
    1340000, 1550000, 1790000, 2050000, 2330000, 2660000, 3060000, 3555000, 4225000, 5130000,
    7050000, 9150000, 11500000, 14500000, 18500000, 22800000, 28100000, 34300000, 41000000, 48000000,
    55100000, 63800000, 73700000, 84000000, 94900000, 106414000, 118043140, 129788571, 141651457, 153632972,
    165734301, 177956644, 190301211, 202769223, 215361915, 228080534, 240926340, 253900603, 267004609, 280370695,
    294004103, 307910179, 322094377, 336562258, 351319497, 366371881, 381725313, 397385813, 413359523, 429652708,
    446271756, 463223185, 480513643, 498149909, 516138902, 534487674, 553203421, 572293483, 591765347, 612210804,
    633678534, 656219650, 679887822, 704739402, 730833562, 758232429, 787001240, 817208492, 848926106, 882229601,
    882229602, 882229603, 882229604, 882229605, 882229606, 882229606, 882229606, 882229606, 882229606, 882229606,
)

#: A warrior's hit points at each level, from the same table. Equipment adds to it:
#: the captured level 1 warrior read 235 of 236 where the table says 225. 2700 at
#: level 15, 9900 at 30, 450000 at 100.
LEVEL_HIT_POINTS = (
    ...
    14310, 18720, 23130, 27540, 31950, 36360, 40770, 45180, 49590, 54000,
    59100, 64200, 69300, 74400, 79500, 84600, 89700, 94800, 99900, 105000,
    117000, 129000, 141000, 153000, 165000, 169000, 173000, 177000, 181000, 185000,
    189000, 193000, 197000, 201000, 205000, 209000, 213000, 217000, 221000, 225000,
    232500, 240000, 247500, 255000, 262500, 270000, 277500, 285000, 292500, 300000,
    307500, 315000, 322500, 330000, 337500, 345000, 352500, 360000, 367500, 375000,
    382500, 390000, 397500, 405000, 412500, 420000, 427500, 435000, 442500, 450000,
...
LEVEL_DAMAGE = (
    ...
    264, 312, 361, 409, 458, 506, 555, 603, 652, 700,
    850, 1000, 1150, 1300, 1450, 1600, 1750, 1900, 2050, 2200,
    2420, 2640, 2860, 3080, 3300, 3500, 3700, 3900, 4100, 4300,
    4500, 4700, 4900, 5100, 5300, 5500, 5700, 5900, 6100, 6300,
    6520, 6740, 6960, 7180, 7400, 7620, 7840, 8060, 8280, 8500,
    8850, 9200, 9550, 9900, 10250, 10600, 10950, 11300, 11650, 12000,
    12480, 12960, 13440, 13920, 14400, 14880, 15360, 15840, 16320, 16800,
